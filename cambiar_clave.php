...
<table border="0" class="table table-bordered">
  <tr class="success">
    <td>
    	<center><strong>
        	<h3><img src="img/log0.jpg" class="img-circle img-polaroid" width="100" height="100"> 
  ...
  <tr>
	...
	if(!empty($_POST['c1']) and !empty($_POST['c2']) and !empty($_POST['contra'])){
		if($_POST['c1']==$_POST['c2']){
			...
			$sql="SELECT * FROM profesor WHERE usu='".$usuario."' and con='".$contra."'";
			$can=mysql_query($sql);
			if($dato=mysql_fetch_array($can)){
				$cnueva=limpiar($nueva,$nit);
				$sql="Update profesor Set con='$cnueva' Where usu='$usuario'";
				mysql_query($sql);
				...
				echo '<div class="alert alert-error">
					  <button type="button" class="close" data-dismiss="alert">×</button>
					  <strong>Contraseña!</strong> Digitada no corresponde a la antigua<br>
					  '.$sql.'
					</div>';
			}
		}else{
